...
def train_loop(ctrl, D, fname, Nround, **kwargs):
    ...
    for round in range(r0, r0 + Nround):
        ...
        imagined_test_rewards = rt_hat[..., Tup:].mean().item()  # 测试的预期奖励
...
def train_policy(ctrl, D, H=2.0, Niter=250, verbose=True, tau=5.0, N=100, L=10, V_const=1.0, save_every=50,
    ...
    for itr in range(Niter):
        ...
        if itr % 100 == 0:
            Vtarget = copy.deepcopy(ctrl.V)  # 每一百次迭代更新价值函数
        s0 = get_ivs(ctrl.env, D, N, rep_buf)  # N,n 初始状态
        noise_vec = ctrl.draw_noise(L)  # 噪声
        fs = ctrl.draw_f(L, noise_vec=noise_vec)  # 得到状态函数的方程
        policy_optimizer.zero_grad()
        # 前向传播，模拟策略的行为并获取t时刻的状态、奖励、动作和时间步
        ...
        gammas = (-ts / tau).exp()  # gamma
        V_st_gam = ctrl.V(st.contiguous())[:, :, 1:, 0] * gammas[1:]
        n_step_returns = rt[:, :, 1:] + V_const * V_st_gam  # ---> n_step_returns[:,:,k] is the sum in (5)
        opimized_returns = n_step_returns.mean(-1)  # L,N 回报的均值
        mean_cost = -opimized_returns.mean()  # 平均成本
        # 反向传播，更新价值函数的参数
        mean_cost.backward()
        ...
        for inner_iter in range(10):
            ...
            value_optimizer.step()  # 根据计算出的梯度更新优化器的参数
